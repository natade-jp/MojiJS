...
const cp932_to_unicode_map = map;
...
const CP932 = {
	 ...
	 */
	toCP932Array: function(text) {
		const utf32 = Unicode.toUTF32Array(text);
		const cp932 = [];
		const ng = "・".charCodeAt(0);
		for(let i = 0; i < utf32.length; i++) {
			const map_bin = unicode_to_cp932_map[utf32[i]];
			if(map_bin) {
				cp932.push(map_bin);
			}
			else {
	 ...
	 */
	toCP932ArrayBinary: function(text) {
