...

{
	...
	test("fromCP932Array", () => { expect(CP932.fromCP932Array(cp932array_).encode_string).toBe(target_text); });
}

{
	const target_text = "ABCあいう高髙①";
	const cp932arraybin = CP932.toCP932Binary(target_text);
	...
	test("fromCP932Array (binary)", () => { expect(CP932.fromCP932Array(cp932arraybin_).encode_string).toBe(target_text); });
}
